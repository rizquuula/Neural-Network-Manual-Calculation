...
wko_new = np.array([wk1o,wk2o])
bo = bo - alpha*Gradloss_bo

print 'New parameter after SGD'
print 'Weight on line hidden layer 2 to output layer: ',wko_new
print 'New Bias: ',bo
Gradloss_kout =[]
...
new_Gradloss_wjk = []
for i in range(len(wjk)):
    new_Gradloss_wjk.append([wjk[i][0]-alpha*Gradloss_wjk[i][0] , wjk[i][1]-alpha*Gradloss_wjk[i][1]])
print 'NEW Gradien Loss in weight of hidden layer 2 to hidden layer 1', new_Gradloss_wjk

new_Gradloss_bjk = []
for i in range(len(bjk)):
    new_Gradloss_bjk.append(bjk[i]-alpha*Gradloss_bjk[i])
print 'NEW Gradien Loss in Bias of Hidden Layer 1 and Hidden Layer 2; ', new_Gradloss_bjk
